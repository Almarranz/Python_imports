...
def plot_two_hists_sigma(table, var1, var2, sym1, sym2, *,
                      bins=50, figsize=(8,4), sig_clip = 3, title1 = None,  title2 = None):
    """
    Plot side-by-side histograms for two columns, with mean and std in the legend.
    ...
    # ---- FIRST ----
    vals1 = np.asarray(table[var1])
    ax.hist(vals1, bins=bins1,color = 'gray', alpha = 0.5,
            label=fr'$\overline{{{sym1}}} = {np.mean(vals1):.2f}$' +'\n'
                  + fr'$\sigma = {np.std(vals1):.2f}$')
    # ax.set_xlabel(fr'${sym1}$ [mas/yr]')
    ax.set_ylabel('# stars')
    ...
    # ---- SECOND ----
    vals2 = np.asarray(table[var2])
    ax2.hist(vals2, bins=bins2, color = 'gray', alpha = 0.5,
             label=fr'$\overline{{{sym2}}} = {np.mean(vals2):.2f}$' +'\n'
                   + fr'$\sigma = {np.std(vals2):.2f}$')
    ax2.set_xlabel(fr'${sym2}$')
    
    ax2.legend()
    
    
    m_var, lim_var = sig_cl(vals1, vals2, sig_clip)
    
    # ---- FIRST ----
    vals1 = np.asarray(table[var1])
    ax.hist(vals1[m_var], bins=bins1, histtype='step',lw =3,
            label=fr'$\overline{{{sym1}}} = {np.mean(vals1[m_var]):.2f}$' +'\n'
                  + fr'$\sigma = {np.std(vals1[m_var]):.2f}$')
    ax.set_xlabel(fr'${sym1}$')
    ax.set_ylabel('# stars')
    ax.axvline(lim_var[0], ls = 'dashed', color = 'red')
    ax.axvline(lim_var[1], ls = 'dashed', color = 'red')
    ax.legend()

    # ---- SECOND ----
    vals2 = np.asarray(table[var2])
    ax2.hist(vals2[m_var], bins=bins2, histtype='step',lw =3,
             label=fr'$\overline{{{sym2}}} = {np.mean(vals2[m_var]):.2f}$' +'\n'
                   + fr'$\sigma = {np.std(vals2[m_var]):.2f}$')
    ax2.set_xlabel(fr'${sym2}$')
    ax2.axvline(lim_var[2], ls = 'dashed', color = 'red')
    ax2.axvline(lim_var[3], ls = 'dashed', color = 'red')
